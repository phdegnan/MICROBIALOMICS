...
my $pname = basename($0);
my $usage= qq{
Usage: $pname [--begin=1-5] [--end=1-5] [--threads #] [--preprocessed] [--debug] <lib_file> <out_base>

Or: $pname [--begin=1-5] [--end=1-5] [--threads #] <Read 1 FastQ> <Read 2 FastQ> <out_base>

Or: $pname [--begin=1-5] [--end=1-5] [--threads #] <Read 1,2 Interleaved FastQ> <out_base>
...
# I/O error #
die $usage if (@ARGV < 2);
die " ERROR: start must be on step 1 or 2\n" if $start !~ /^[12]$/;
...
if ($start <= 2) {
	...
	else{		# if started on step 1 #
		my $fq_reads = "$OUTBASE.ec.fastq";
		}
	# gunzipping input reads #
	if (-f "$fq_reads.gz" && ! -f "$fq_reads") { 
		`gunzip $fq_reads.gz`;			# unziping reads if gzipped
	}
